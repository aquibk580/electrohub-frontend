...
const Navbar = () => {
  ...
  const components: { title: string; img: string; href: string; description: string }[] = [
    {
      title: "Alert Dialog",
      img: assets.laptop,
  ...
  return (
    <header>
      <div className="bg-green-950 text-white w-full items-start text-sm flex justify-between px-6 py-1 ">
        <div className="flex items-center"><img className="w-4" src={assets.phone_icon} alt="" /><p>+91-987654321</p></div>
      ...
      <nav className=" w-ful border  flex justify-evenly px-12 py-4 font-medium items-center">
        ...
        <NavigationMenu>
          <NavigationMenuList>
            <NavigationMenuItem>
              <NavigationMenuTrigger className="rounded-full">Deals</NavigationMenuTrigger>
              <NavigationMenuContent>
                  ...
                  {components.map((component) => (
                    ...
                    >
                      <div className="flex flex-row items-center gap-3">
                        <img src={component.img} alt={component.title} className="w-20 h-20 rounded-md object-cover" />

                        <div className="flex flex-col items-start">
                          <div className="font-medium text-black text-md">{component.title}</div>
                          <div className="text-xs">{component.description.slice(0, 45)}...</div>
                        </div>
                      </div>
                    </ListItem>
                  ))}
        ...
        <div className="flex gap-6">
          ...
          <div onClick={() => navigate("/user/auth/login")} className="flex gap-1 items-center  cursor-pointer hover:text-orange-600 ">
            <img className="h-5" src={assets.account_icon} alt="" />
            <p>Account</p>
          </div>
          <div className="flex gap-1 items-center  cursor-pointer hover:text-orange-600">
            <img className="h-5" src={assets.cart_icon} alt="" />
            Cart
