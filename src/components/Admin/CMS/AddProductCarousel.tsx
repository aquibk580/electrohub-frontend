...
const productCarouselSchema = z.object({
  ...
  image: z
    ...
    .refine((file) => file.size > 0, "Uploaded file is invalid"),
...
type BannerCarouselSchemaType = z.infer<typeof productCarouselSchema>;
...
export default function AddProductCarousel({
  ...
  const onSubmit = async (data: BannerCarouselSchemaType) => {
    try {
      ...
      if (response.status === 201) {
        ...
        toast.success(
          `Slider Product ${response.data?.productCarousel?.name} Added Successfully`,
          {
            position: "top-center",
            theme: "dark",
          }
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Add New Product for slider</DialogTitle>
        </DialogHeader>
        <FormProvider {...methods}>
          ...
          >
            ...
            <div className="grid w-full items-center gap-1.5">
              <Label>Price</Label>
              <Input {...methods.register("price")} placeholder="Enter price" />
              {errors.price && (
                <p className="text-red-500 text-sm">
                ...
                render={({ field }) => (
                  <FormItem className="flex flex-row items-center justify-between rounded-lg border p-3 shadow-sm">
                    <div className="space-y-0.5">
                      <FormLabel>Enable Product</FormLabel>
                      <FormDescription>
                        Toggle to enable or disable the product.
                      </FormDescription>
                    </div>
                    <FormControl>
                      <Switch
