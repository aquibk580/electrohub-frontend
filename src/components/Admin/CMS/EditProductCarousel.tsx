...
import { DollarSign, Edit, ExternalLink, ImagePlus, Loader2 } from 'lucide-react'
...
export default function EditProductCarouselButton({
  ...
  const [preview, setPreview] = useState<string | null>(productCarousel.imageUrl)
  ...
  const onSubmit = async (data: ProductCarouselSchemaType) => {
    try {
      ...
      const response = await axios.patch(
        `${import.meta.env.VITE_API_URL}/api/admin/cms/product-carousels/${productCarousel.id}`,
        formData
      )
